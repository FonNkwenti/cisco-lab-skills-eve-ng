...
mkdir -p "$TARGET_DIR"/{.agent,.claude/commands,blueprint/"$EXAM_CODE"/references,specs,docs,labs,tests,.prompts,tasks}
...
cp "$HUB_DIR/conductor-template/workflow.md"       "$TARGET_DIR/WORKFLOW.md"
cp "$HUB_DIR/conductor-template/tech-stack.md"     "$TARGET_DIR/docs/tech-stack.md"
cp -r "$HUB_DIR/conductor-template/code_styleguides" "$TARGET_DIR/docs/code_styleguides"

sed "s/{{CERT_NAME}}/$CERT_NAME/g; s/{{EXAM_CODE}}/$EXAM_CODE/g; s/{{SCOPE}}/TODO: Define scope of coverage/g" \
  "$HUB_DIR/conductor-template/product.md.tmpl" > "$TARGET_DIR/docs/product.md"

sed "s/{{CERT_NAME}}/$CERT_NAME/g; s/{{EXAM_CODE}}/$EXAM_CODE/g" \
  "$HUB_DIR/conductor-template/product-guidelines.md.tmpl" > "$TARGET_DIR/docs/product-guidelines.md"
...
Additional commands: \`/build-capstone\`, \`/tag-lab\`, \`/sync-skills\`, \`/project-status\`, \`/audit-faults\`, \`/validate-lab\`. All commands live in \`.claude/commands/\` — inspect or customise them there. The commands are advisory: they warn on missing prerequisites but let you proceed. See \`COMMANDS.md\` for the full list and \`WORKFLOW.md\` for the end-to-end process.
